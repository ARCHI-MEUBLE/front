    ...
    <div className="flex min-h-screen flex-col bg-alabaster text-ink">
      ...
      <main className="flex flex-1 flex-col">
        <section className="mx-auto w-full max-w-5xl px-6 py-20">
          <h1 className="heading-serif text-4xl text-ink">Votre sélection d’échantillons</h1>
          ...
          {isEmpty ? (
            <div className="mt-16 flex flex-col items-center rounded-[40px] border border-dashed border-[#d7c9b9] bg-white/70 px-12 py-16 text-center shadow-sm">
              <p className="heading-serif text-2xl text-ink">Votre panier est vide.</p>
              ...
              <Link
                href="/#couleurs"
                className="mt-8 button-elevated"
              >
            ...
            <div className="mt-16 space-y-6">
              {items.map((item) => (
                <div
                  key={item.id}
                  className="flex flex-col gap-6 rounded-[32px] border border-[#e0d7cc] bg-white/80 p-8 shadow-sm transition hover:-translate-y-1 hover:shadow-xl sm:flex-row"
                >
                  ...
                  <div className="flex flex-1 flex-col justify-between">
                    <div>
                      <h2 className="heading-serif text-2xl text-ink">{item.name}</h2>
                      <p className="mt-3 text-sm leading-relaxed text-ink/70">{item.description}</p>
                    </div>
                    <button
                      type="button"
                      onClick={() => removeItem(item.id)}
                      className="mt-8 self-start rounded-full border border-[#d7c9b9] px-5 py-2 text-sm font-semibold uppercase tracking-[0.2em] text-ink/70 transition hover:border-ink hover:text-ink"
                    >
                      Retirer du panier
                    </button>
                  </div>
        ...
        <section className="mt-auto bg-white/60 py-14">
          <div className="mx-auto flex max-w-5xl flex-col items-center justify-between gap-6 px-6 text-center sm:flex-row sm:text-left">
            <div>
              <p className="heading-serif text-xl text-ink">3 échantillons maximum — Livraison offerte</p>
